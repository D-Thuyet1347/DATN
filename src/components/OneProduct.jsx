...
const OneProduct = ({ title, price, description, image, productId, onAddToCart }) => {
 ...
 <div className="absolute top-3  right-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
    <Tooltip  placement="leftTop" title='Thêm sản phẩm' arrow={mergedArrow}>
    <FaCartPlus 
        onClick={() => onAddToCart(productId)}
        className="h-[28px] mt-[10px] w-[28px] border border-black opacity-60" 
      />
      </Tooltip>
    <Tooltip  placement="leftTop" title='Xem chi tiết sản phẩm' arrow={mergedArrow}>
    <Link to={`/product/${productId}`}><IoEyeOutline  
    className="h-[28px] mt-[10px] w-[28px]  border border-black opacity-60" /></Link>
    </Tooltip>
    </div>
 ...
 <div className="absolute left-[-20px] top-[30px] mt-2 bg-slate-500 h-[60px] w-[350px] m-auto text-center">
  <button onClick={() => onAddToCart(productId)} className="h-[40px] w-[350px] text-white ">
  Thêm vào giỏ hàng
  </button>
</div>
