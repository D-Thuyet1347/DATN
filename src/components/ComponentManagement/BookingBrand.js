...
const BookingBrand = () => {
      ...
      try {
        ...
        if (Array.isArray(response)) {
          const filtered = response.filter(
            (booking) => booking.branch?._id === managerBranchId
          );
  
          const processedBookings = filtered.map((item) => ({
            ...item,
            key: item._id,
    ...
    const columns = [
      ...
      },
      {
        ...
        key: "branch",
        render: (_, record) => record.branch?.BranchName || "Không rõ",
      },
      {
        title: "Ngày hẹn",
            ...
            {state.selectedBooking ? (
              <Descriptions bordered column={1} size="small">
                ...
                <Descriptions.Item label="Chi nhánh">
                  {state.selectedBooking.branch?.BranchName}
                </Descriptions.Item>
